...
export async function ExtractDataWithAIExecutor(environment: ExecutionEnvironment<typeof ExtractDataWithAITask>): Promise<boolean> {
    try {
        ...
        environment.log.info(`Prompt tokens: ${response.response.usageMetadata?.promptTokenCount}`);
        environment.log.info(`Completition tokens: ${response.response.usageMetadata?.candidatesTokenCount}`);
